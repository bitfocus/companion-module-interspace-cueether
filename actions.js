		...
		send_cue_action: {
			...
			callback: async(event) => {
				...
				if (event.options.channel.length >= 4) {
					self.buildSendPacket(event.options.cueType, 5); // Sends to all channels
				} else {
					for (let _i = 0; _i < event.options.channel.length; _i++) {
						self.buildSendPacket(event.options.cueType, event.options.channel[_i]); // Sends to specific channel
					}
				}
			}
		},
